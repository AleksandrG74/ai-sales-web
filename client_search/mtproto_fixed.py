...
class MTProtoSearcher:
    ...
    async def connect(self):
        """Подключение к Telegram через MTProto прокси"""
        session_path = os.path.join(os.path.dirname(__file__), '..', 'sessions', 'telegram_session')
        os.makedirs(os.path.dirname(session_path), exist_ok=True)
        
        try:
            # Создаём клиент с MTProto прокси
            # (server, port, secret) для MTProto
            ...
            logger.info("🔒 Подключение через MTProto прокси 127.0.0.1:1443")
